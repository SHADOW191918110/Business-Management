...
const Inventory = (() => {
  ...
  async function saveProduct() {
    ...
    await API.createProduct(body); closeModal(); await load(); POS.loadProducts();
  }

  function filterData(list) {
    const q = els.search.value.trim().toLowerCase();
    const cat = els.filter.value;
    return list.filter(p => (cat === 'all' || p.category === cat) && (!q || p.name.toLowerCase().includes(q)));
  }
  ...
  function bind() {
    ...
    document.querySelector('#add-product-modal .close').addEventListener('click', closeModal);
...
document.addEventListener('DOMContentLoaded', Inventory.bind);
