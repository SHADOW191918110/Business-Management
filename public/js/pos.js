...
const POS = (() => {
  ...
  function renderProducts(list) {
    ...
    [...document.querySelectorAll('.product-card')].forEach(card => card.addEventListener('click', () => addToCart(card.dataset.id)));
  }
  ...
  async function completeSale() {
    const totals = recalcTotals();
    const amountReceived = parseFloat(els.amountReceived.value || '0');
    const change = +(amountReceived - totals.total).toFixed(2);
    els.changeAmount.textContent = `₹${Math.max(change, 0).toFixed(2)}`;
  ...
  function bindEvents() {
    ...
    document.querySelectorAll('.modal .close').forEach(el => el.addEventListener('click', (e) => closeModal(e.target.dataset.modal)));
  ...
  return {
    async loadProducts() {
      try {
        ...
        state.products = data; state.filtered = data; renderProducts(data);
      } catch (err) { console.error(err); }
      bindEvents();
    },
    recalculate: recalcTotals,
    search(q) { filterProducts(); }
  };
})();
